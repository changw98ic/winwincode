...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    throw new Error(`unknown Community build argument: ${argument}`)
  }
...
if (result.status !== 0 || result.signal !== null) {
    throw new Error(`Community build failed with ${result.signal ?? `exit code ${result.status}`}`)
}
...
const artifacts = PRODUCT_TARGETS.map(product => {
  ...
  if (!existsSync(path) || !statSync(path).isFile()) {
    throw new Error(`Community build did not produce ${path}`)
  }
  return `${product.binaryName}:${path}`
})
process.stdout.write(
  `built ${profile} Community runtime (${artifacts.join(', ')}) and ${LOCAL_PACKAGE_NAME}\n`,
)
