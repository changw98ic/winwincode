...
function rustType(schema, document, context, qualifyShared = false) {
  ...
  if (schema.const !== undefined) {
    const declaredTypes = schemaTypes(schema)
    if (declaredTypes.length === 1) return rustPrimitiveType(declaredTypes[0])
    return rustPrimitiveType(typeof schema.const)
  }
...
function rustJsonPresentValueConstraint(
  ...
  if (schema.const !== undefined) {
    ...
    if (typeof schema.const === 'number' && schemaTypes(schema).includes('integer')) {
      return `${valueAccessor}.as_i64() == Some(${rustIntegerLiteral(schema.const, sourcePath)})`
    }
