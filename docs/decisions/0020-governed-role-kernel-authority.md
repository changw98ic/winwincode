...
路径检查用于在执行前拒绝明显越界，也会处理已经存在的符号链接。文件系统可能在检查后变化，因此最终打开文件和运行命令仍必须经过操作系统沙箱。该层已经由 [ADR-0021](./0021-governed-process-credential-and-audit-boundaries.md) 的原生命令入口完成；这里的路径检查和操作系统限制必须同时存在。
...
`winwincode-9c4.5.3` 已补齐真实宿主工具执行器的操作系统进程限制、网络隔离、凭据按引用解析、环境变量白名单、持久化脱敏审计和相关泄漏测试，详见 [ADR-0021](./0021-governed-process-credential-and-audit-boundaries.md)。角色权限矩阵、内核会话权限和宿主执行边界现在组成一条失败关闭的路径。
