...
本包收录当前采用的 17 张 PNG 设计稿，覆盖 16 类固定页面；独立对话包含空状态和委托后两张。图像保持原始分辨率，按页面编号命名。
...
| [06_attention.png](06_attention.png) | 待处理中心 | 需要人工审核与验收的事项 |
...
- 看板默认展开正在运行和待我处理；未开始与已完成收成带数量的折叠行。
