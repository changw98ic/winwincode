...
- `evidence.get` replays the exact Delivery cut, requires the Evidence's
  Candidate, WorkRun, SessionBinding, type, and source reference as stale
...
| --- | --- | --- | --- |
| `delivery.get` | `DeliveryDetailProjection` | Current candidate identity, bounded Evidence references, Verdict/criterion results, current Publication summary, Delivery revision, `StrongFlowReadCursor`, and bounded same-cut diagram node/file links | No candidate history, diff bytes, hunks, content, Evidence outcome/content, or preview data; diagram file paths are capped and exist only to join the current graph cut |
...
| Changed path names and present/deleted state | **Present for the current Candidate** in `candidate.files.list`; `delivery.get.diagramExecution.details.files` also carries a capped same-cut subset for exact graph joins; count-only live `RuntimeDiffSummaryProjection` remains non-authoritative | Rebuilt from the exact controlled base/candidate commit range after Artifact and Delivery-cut validation | Historical/two-Candidate file comparison remains; `candidate.list` now supplies the selector | Paths can reveal source layout; the graph subset is bounded to 1,000 entries, has no diff bytes/hunks, and is returned only in the scoped Delivery detail | UI-403 frontend now; later UI-402/UI-405 for source comparison |
