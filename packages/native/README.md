...
- `codex-resources/bwrap`：仅 Linux 提供，确保全新安装不依赖系统预装 bubblewrap；
...
Linux 构建机需要 `binutils`、`pkg-config` 和 `libcap` 开发文件。原生发布 CI 不随提交自动运行；手动选择 `linux` 时只跑两个 Linux 目标，确认全绿后再手动选择 `macos` 做一次回归检查。
