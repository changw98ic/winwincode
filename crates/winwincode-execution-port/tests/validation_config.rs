...
// The canonical repository configuration is embedded so the contract test
// stays hermetic: `.winwincode/validation.toml` is a local instance file and
// is never committed.
const CONFIGURATION: &str = r#"schemaVersion = 1
...
argv = ["cargo", "check", "--message-format=json", "--workspace", "--all-targets", "--all-features", "--locked"]
workingDirectory = "."
allowedCompanionPaths = []
environment = []
network = false
...
phase = "validation"
language = "python"
argv = ["python3", "-c", "import ast,pathlib; ast.parse(pathlib.Path('third_party/codex/sdk/python/release_version.py').read_text(encoding='utf-8'))"]
workingDirectory = "."
allowedCompanionPaths = []
environment = []
network = false
...
language = "rust"
diagnosticParserVersion = "cargo_json_v1"
argv = ["cargo", "test", "--message-format=json", "--workspace", "--all-features", "--locked"]
workingDirectory = "."
allowedCompanionPaths = []
environment = []
network = false
...
[[profiles]]
name = "affected"
commandIds = ["rust-format", "typescript-check", "rust-check", "python-syntax"]

[[profiles]]
name = "final"
commandIds = ["rust-format", "typescript-check", "rust-tests", "python-syntax"]
"#;
