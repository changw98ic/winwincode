...
use sha2::Sha256;
use winwincode_connector_linear::{
    LINEAR_CONNECTOR_PROTOCOL, LinearClock, LinearConnector, LinearConnectorConfig,
    LinearConnectorScope, LinearCredentialError, LinearCredentialPort, LinearEventAction,
    LinearEventKind, LinearEventMapperPort, LinearInboundEvent, LinearOAuthScope, LinearOAuthToken,
    LinearProjectId, LinearTeamId, LinearTlsRoots, LinearWebhookHeaders,
    LinearWebhookRequestFactory, LinearWebhookSecret, LinearWebhookVerifier, LinearWorkspaceId,
};
...
use winwincode_integration_core::{
    ...
    IntegrationScope, NormalizedInboundEvent, OutboundAttemptResult, OutboundOperationState,
...
fn signed_webhooks_are_closed_scoped_replay_safe_and_secret_free() {
    ...
    let mut connector =
        LinearConnector::try_new(config.clone(), credentials, calls, FixedClock(1_000))
            .expect("connector");
...
fn webhook_timestamp_header_scope_and_revocation_fail_closed() {
    ...
    let mut connector = LinearConnector::try_new(
        config.clone(),
        credentials.clone(),
...
fn connector_for_fixture(
    ...
    clock: u64,
) -> LinearConnector<CredentialFixture, MapperFixture, FixedClock> {
    let config = base_config(
        fixture.endpoint.clone(),
        LinearTlsRoots::Specific(vec![fixture.certificate_der.clone()]),
    );
    LinearConnector::try_new(
        config,
...
fn restart_after_remote_comment_write_finds_marker_without_duplicate_mutation() {
    ...
    restarted
        .storage_mut()
        .record_success(&integration_scope(), &recovered, &found, 1_006)
