...
const USER_AGENT: &str = "WinWinCode-Linear-Connector";
...
pub struct LinearConnector<Credentials, Mapper, Clock> {
    config: LinearConnectorConfig,
    credentials: Credentials,
    mapper: Mapper,
    clock: Clock,
...
impl<Credentials, Mapper, Clock> LinearConnector<Credentials, Mapper, Clock> {
...
impl<Credentials: LinearCredentialPort, Mapper: LinearEventMapperPort, Clock: LinearClock>
    ConnectorPort for LinearConnector<Credentials, Mapper, Clock>
{
...
impl<Credentials, Mapper, Clock: LinearClock> LinearConnector<Credentials, Mapper, Clock> {
...
fn require_authority(
    ...
    if matches_authority(config, authority)
        && authority.state() == winwincode_integration_core::ConnectorState::Active
    {
        Ok(())
    } else if authority.state() == winwincode_integration_core::ConnectorState::CredentialRevoked {
        Err(connector_error(
            ConnectorCallErrorKind::CredentialRevoked,
            "LINEAR_CREDENTIAL_REVOKED",
        ))
