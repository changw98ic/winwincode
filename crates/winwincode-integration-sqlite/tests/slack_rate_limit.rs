...
use winwincode_connector_slack::{
    ...
    SlackWebApiMethod, SlackWorkspaceId,
};
...
use winwincode_integration_core::{
    ...
    IntegrationOperationKey, IntegrationScope, OutboundClaim, OutboundRequest, RetryPolicy,
};
...
fn connector(
    ...
    directory: &PathBuf,
) -> SlackConnector<CredentialFixture, ClockFixture> {
...
fn reconciliation_rejects_foreign_team_app_and_bot_before_accepting_exact_installation() {
    ...
    for (index, request) in requests.iter().enumerate() {
        ...
        storage
            .record_success(&integration_scope(), &claim, &receipt, 200 + index as u64)
