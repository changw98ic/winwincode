...
use winwincode_integration_core::{
    ...
    IntegrationOperationKey, IntegrationScope, NormalizedInboundEvent, OutboundAttemptResult,
...
fn integration_scope(organization: char, repository: char) -> IntegrationScope {
    IntegrationScope::try_new(format!("scope:{organization}:{repository}")).expect("scope")
}
...
impl ConnectorPort for ConnectorFixture {
    ...
    fn deliver_outbound(
        &mut self,
        claim: &winwincode_integration_core::OutboundClaim,
...
fn assert_inbound_audit_is_secret_safe(
    ...
    let audit = framework
        .storage()
        .audit_facts(scope, integration_id, 0, 20)
        .expect("audit facts");
    assert_eq!(
        audit
            .iter()
            .map(winwincode_integration_core::IntegrationAuditFact::kind)
            .collect::<Vec<_>>(),
        vec![
...
fn deliver_permanent_failure(
    ...
    let result = framework
        .deliver_next(scope, integration_id, 106, lease('M'), 111, connector)
...
fn inbound_signature_replay_ordering_and_audit_are_secret_safe_across_restart() {
    ...
    let registered = framework
        .register_connector(&registration(integration_id.clone(), scope.clone(), '5'))
        .expect("register");
    ...
    let dispatches = framework
        .storage()
        .inbound_dispatches(&scope, &integration_id, 0, 10)
        .expect("dispatches");
    ...
    assert_inbound_audit_is_secret_safe(&framework, &scope, &integration_id);
...
#[test]
fn invalid_signature_and_foreign_scope_have_zero_dispatch_side_effects() {
    let root = temporary_directory("signature");
    let integration_id = integration_id('B');
    let scope = integration_scope('1', '4');
    let foreign = integration_scope('6', '7');
    let mut framework = IntegrationFramework::new(IntegrationStorage::open(&root).expect("store"));
    framework
        .register_connector(&registration(integration_id.clone(), scope.clone(), '5'))
        .expect("register");
    ...
    assert_eq!(
        framework
            .register_connector(&registration(integration_id.clone(), scope.clone(), '6'))
    ...
    assert_eq!(
        framework
            .storage()
            .audit_facts(&scope, &integration_id, 0, 10)
            .expect("audit")
            .len(),
        1
    );
...
fn verifier_credential_revocation_durably_stops_later_webhook_calls() {
    ...
    assert_eq!(
        framework
            .storage()
            .authority(&scope, &integration_id)
...
fn concurrent_duplicate_webhooks_create_one_formal_command_dispatch() {
    ...
    bootstrap
        .register(&registration(integration_id.clone(), scope.clone(), '5'))
        .expect("register");
    drop(bootstrap);
...
fn outbound_retry_uses_one_idempotency_key_and_delivers_after_capped_backoff() {
    ...
    assert_outbound_retry_audit(&framework, &scope, &integration_id);
    drop(framework);
    fs::remove_dir_all(root).expect("cleanup");
}
...
fn lease_recovery_reuses_operation_and_exact_terminal_receipt() {
    ...
    let abandoned = storage
        .claim_due(&scope, &integration_id, 100, lease('D'), 105)
        .expect("claim")
        .expect("due");
    ...
    assert!(
        restarted
            .claim_due(&scope, &integration_id, 104, lease('E'), 110)
            .expect("live lease")
            .is_none()
    );
    let recovered = restarted
        .claim_due(&scope, &integration_id, 105, lease('F'), 115)
    ...
    let applied = restarted
        .record_success(&scope, &recovered, &remote, 106)
        .expect("success");
    assert!(!applied.idempotent_replay());
    let replay = restarted
        .record_success(&scope, &recovered, &remote, 999)
...
fn failure_receipt_replay_requires_the_same_failure_category() {
    ...
    storage.enqueue_outbound(&request).expect("enqueue");
    let claim = storage
        .claim_due(&scope, &integration_id, 100, lease('K'), 105)
        .expect("claim")
        .expect("due claim");
    let retryable = connector_error(ConnectorCallErrorKind::Retryable, "REMOTE_BUSY");
    assert!(matches!(
        storage
            .record_failure(&scope, &claim, &retryable, 100)
            .expect("record retry"),
        OutboundAttemptResult::RetryScheduled(_)
    ));
    assert!(matches!(
        storage
            .record_failure(&scope, &claim, &retryable, 999)
...
fn exhausted_and_permanent_failures_dead_letter_without_business_mutation() {
    ...
    let permanent_request =
        deliver_permanent_failure(&mut framework, &mut connector, &scope, &integration_id);
    assert_eq!(
        framework
            .storage()
            .inbound_dispatches(&scope, &integration_id, 0, 10)
    ...
    assert_eq!(
        restarted
            .outbound_operation(&scope, &integration_id, request.operation_key())
            .expect("operation")
            .state(),
        OutboundOperationState::DeadLetter
    );
    assert_eq!(
        restarted
            .outbound_operation(&scope, &integration_id, permanent_request.operation_key())
...
fn credential_revocation_stops_inbound_and_outbound_before_connector_calls() {
    ...
    let revoked = framework
        .revoke_credential(&scope, &integration_id, authority.revision(), 101)
        .expect("revoke");
    ...
    assert_eq!(
        framework
            .revoke_credential(&scope, &integration_id, authority.revision(), 102)
