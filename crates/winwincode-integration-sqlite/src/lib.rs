...
//! Community `SQLite` integration authority, receipts, queues, and rate limits.

mod slack_rate_limit;

pub use slack_rate_limit::SlackRateLimitGate;
...
use winwincode_integration_core::model::{
    MAX_SAFE_INTEGER, corrupt, domain_digest, invalid, scope_bytes, validate_count,
    validate_digest, validate_integration_id, validate_scope, validate_time,
};
use winwincode_integration_core::{
    ...
    InboundReceipt, InboundStatus, InboundWebhookRequest, IntegrationAuditFact,
    IntegrationAuditKind, IntegrationError, IntegrationErrorKind, IntegrationId,
...
impl IntegrationStorage {
    ...
    /// # Errors
    ///
    /// Rejects changed connector reuse, scope conflict, or storage failure.
    pub fn register(
        &mut self,
    ...
    /// Loads the exact scoped connector authority.
    ///
    /// # Errors
    ///
    /// Rejects missing, foreign scope, or corrupt authority.
    pub fn authority(
        &self,
    ...
    /// # Errors
    ///
    /// Rejects foreign scope, stale revision, invalid time, or storage failure.
    ...
    /// # Errors
    ///
    /// Rejects stale authority, changed event reuse, scope mismatch, revoked
    ...
    /// # Errors
    ///
    /// Rejects foreign scope, invalid cursor/limit, corrupt data, or storage failure.
    ...
    /// # Errors
    ///
    /// Rejects revoked credentials, changed operation-key reuse, stale scope,
    /// or storage failure.
    ...
    /// # Errors
    ///
    /// Rejects changed outcome reuse, stale claims, scope mismatch, or storage failure.
    ...
    /// # Errors
    ///
    /// Rejects changed failure reuse, stale claims, scope mismatch, or storage failure.
    ...
    /// Reads current outbound state within one scope.
    ///
    /// # Errors
    ///
    /// Rejects foreign scope, missing operation, corrupt state, or storage failure.
    ...
    /// Scans a bounded secret-safe audit-outbox page for one scoped connector.
    ///
    /// # Errors
    ///
    /// Rejects foreign scope, invalid cursor/limit, corrupt facts, or storage failure.
    pub fn audit_facts(
        &self,
        scope: &IntegrationScope,
        integration_id: &IntegrationId,
...
impl IntegrationStoragePort for IntegrationStorage {
    fn register(
        &mut self,
        registration: &ConnectorRegistration,
    ) -> Result<ConnectorRegistrationReceipt, IntegrationError> {
        IntegrationStorage::register(self, registration)
    }
    ...
    fn authority(
        ...
        IntegrationStorage::authority(self, scope, integration_id)
    }
    ...
    fn revoke_credential(
        ...
        IntegrationStorage::revoke_credential(
            self,
            scope,
            integration_id,
            expected_revision,
    ...
    fn accept_inbound(
        ...
        IntegrationStorage::accept_inbound(self, expected_authority, request, normalized)
    }
    ...
    fn inbound_dispatches(
        ...
        IntegrationStorage::inbound_dispatches(self, scope, integration_id, after_sequence, limit)
    }
    ...
    fn enqueue_outbound(
        ...
        IntegrationStorage::enqueue_outbound(self, request)
    }
    ...
    fn record_success(
        ...
        IntegrationStorage::record_success(self, scope, claim, remote, completed_at_millis)
    }
    ...
    fn record_failure(
        ...
        IntegrationStorage::record_failure(self, scope, claim, failure, failed_at_millis)
    }
    ...
    fn outbound_operation(
        ...
        IntegrationStorage::outbound_operation(self, scope, integration_id, operation_key)
    }
    ...
    fn audit_facts(
        ...
        IntegrationStorage::audit_facts(self, scope, integration_id, after_sequence, limit)
    }
}
...
fn load_inbound_receipt(
    ...
    row.map(
        |(ordering_key, payload, sequence, status, command, received_at)| {
            Ok(InboundReceipt::from_stored(
                winwincode_integration_core::model::StoredInboundReceipt {
...
fn require_page(after_sequence: u64, limit: usize) -> Result<(), IntegrationError> {
    if after_sequence > MAX_SAFE_INTEGER || !(1..=500).contains(&limit) {
        Err(invalid())
    } else {
        Ok(())
    }
...
fn stored_scope_value(bytes: &[u8]) -> Result<IntegrationScope, IntegrationError> {
    let scope: IntegrationScope = serde_json::from_slice(bytes).map_err(|_| corrupt())?;
...
const fn scope_error() -> IntegrationError {
    IntegrationError::new(
        IntegrationErrorKind::ScopeMismatch,
        "integration scope does not match",
    )
}
