...
use winwincode_connector_slack::{
    ...
    SlackChannelId, SlackConnector, SlackConnectorConfig, SlackCredentialError,
...
use winwincode_integration_core::{
    ...
    IntegrationScope, OutboundAttemptResult, OutboundRequest, RetryPolicy,
};
...
fn run_slack_live_gate(
    ...
    let mut connector = SlackConnector::try_new(
        config.clone(),
        credentials.clone(),
    ...
    let dispatches = framework
        .storage()
        .inbound_dispatches(&integration_scope(), &integration, 0, 10)
        .map_err(|_| GateFailure::DurableStateRejected)?;
    ...
    let authority = framework
        .storage()
        .authority(&integration_scope(), &integration)
        .map_err(|_| GateFailure::DurableStateRejected)?;
    framework
        .revoke_credential(
            &integration_scope(),
    ...
    if revoked_result
        .as_ref()
        .err()
        .map(winwincode_integration_core::IntegrationError::kind)
    ...
    let audits = framework
        .storage()
        .audit_facts(&integration_scope(), &integration, 0, 100)
        .map_err(|_| GateFailure::DurableStateRejected)?;
...
fn integration_scope() -> IntegrationScope {
    IntegrationScope::try_new("scope:3:6").expect("static Slack live-gate scope")
}
...
fn deliver_until_terminal(
    ...
    connector: &mut SlackConnector<LiveCredentialPort, SystemSlackClock>,
) -> Result<(), GateFailure> {
...
fn validate_single_control_plane_command(
    dispatches: &[winwincode_integration_core::InboundDispatch],
    expected_revision: u64,
    expires_at_millis: u64,
) -> Result<(), GateFailure> {
