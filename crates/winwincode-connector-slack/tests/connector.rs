...
use winwincode_connector_slack::{
    ...
    SlackChannelId, SlackClock, SlackConnector, SlackConnectorConfig, SlackCredentialError,
    SlackCredentialPort, SlackInstallationIdentity, SlackSigningSecret, SlackTlsRoots,
...
use winwincode_integration_core::{
    ...
    IntegrationScope, OutboundAttemptResult, OutboundOperationState, OutboundRequest, RetryPolicy,
};
use winwincode_integration_sqlite::{IntegrationStorage, SlackRateLimitGate};
...
fn slack_connector<Credentials>(
    ...
    directory: &PathBuf,
) -> SlackConnector<Credentials, ClockFixture> {
    SlackConnector::try_new(
        config,
        credentials,
...
fn url_button_is_idempotently_acknowledged_empty_within_slack_budget() {
    ...
    assert_eq!(
        factory
            .accept(integration_scope(), &headers, body.clone(), 0)
            .expect_err("zero acknowledgment time")
            .kind(),
        IntegrationErrorKind::Invalid
    );
    let first = factory
        .accept(integration_scope(), &headers, body.clone(), NOW_MILLIS)
        .expect("open callback");
    let replay = factory
        .accept(integration_scope(), &headers, body, NOW_MILLIS)
...
fn signed_callbacks_are_revision_bound_expiring_replay_safe_and_secret_safe() {
    ...
    assert_secret_safe(&framework, &directory);
    drop(framework);
...
fn assert_callback_dispatch(
    ...
    let dispatches = framework
        .storage()
        .inbound_dispatches(&integration_scope(), &integration_id(), 0, 10)
        .expect("dispatches");
...
fn assert_changed_callback_conflicts(
    ...
    connector: &mut SlackConnector<CredentialFixture, ClockFixture>,
) {
...
fn assert_secret_safe(framework: &IntegrationFramework, directory: &Path) {
    let audit = framework
        .storage()
        .audit_facts(&integration_scope(), &integration_id(), 0, 20)
        .expect("audit facts");
