...
/// Product-neutral scope identity supplied by the composing application.
#[derive(Clone, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(transparent)]
pub struct IntegrationScope(String);

impl IntegrationScope {
    /// Builds one bounded portable scope identity.
    ///
    /// # Errors
    ///
    ...
    /// Returns the opaque scope identity without assigning product semantics.
    #[must_use]
...
impl IntegrationOperationKey {
    ...
    pub fn from_stored(value: Sha256Digest) -> Result<Self, IntegrationError> {
...
impl ConnectorRegistration {
    /// Builds one scoped connector registration.
    ///
    /// # Errors
    ///
    /// Rejects invalid scope, credential reference, or time facts.
    pub fn try_new(
        integration_id: IntegrationId,
        scope: IntegrationScope,
        protocol: ConnectorProtocol,
    ...
    #[must_use]
    pub const fn scope(&self) -> &IntegrationScope {
        &self.scope
    }
    #[must_use]
...
impl ConnectorRegistrationReceipt {
    pub const fn new(authority: ConnectorAuthority, idempotent_replay: bool) -> Self {
        Self {
            authority,
            idempotent_replay,
        }
...
impl InboundWebhookRequest {
    ...
    #[must_use]
    pub fn event_key(&self) -> Sha256Digest {
        domain_digest(
    ...
    #[must_use]
    pub fn ordering_key_digest(&self) -> Sha256Digest {
        domain_digest(
...
impl InboundNormalizationContext {
    pub fn from_request(request: &InboundWebhookRequest) -> Self {
        Self {
...
#[doc(hidden)]
pub struct StoredInboundReceipt {
...
impl InboundReceipt {
    pub fn from_stored(stored: StoredInboundReceipt) -> Self {
        Self {
    ...
    #[must_use]
    pub const fn with_replay(mut self, idempotent_replay: bool) -> Self {
...
impl RetryPolicy {
    ...
    pub fn retry_at(self, attempt: u32, failed_at: u64) -> Result<u64, IntegrationError> {
...
impl OutboundRequest {
    ...
    /// # Errors
    ///
    /// Rejects invalid scope, name, payload, or time facts.
    pub fn try_new(
        integration_id: IntegrationId,
        scope: IntegrationScope,
        operation_key: IntegrationOperationKey,
...
impl OutboundEnqueueReceipt {
    pub const fn new(operation: OutboundOperation, idempotent_replay: bool) -> Self {
        Self {
            operation,
            idempotent_replay,
        }
...
/// Secret-safe durable audit fact. It contains only scope identity, connector
...
#[doc(hidden)]
pub fn validate_scope(scope: &IntegrationScope) -> Result<(), IntegrationError> {
    IntegrationScope::try_new(scope.as_str()).map(|_| ())
}

#[doc(hidden)]
pub fn validate_integration_id(integration_id: &IntegrationId) -> Result<(), IntegrationError> {
    validate_id(&integration_id.0, "int")
}

#[doc(hidden)]
pub fn scope_bytes(scope: &IntegrationScope) -> Result<Vec<u8>, IntegrationError> {
    validate_scope(scope)?;
    serde_json::to_vec(scope).map_err(|_| corrupt())
}

#[doc(hidden)]
pub fn validate_time(value: u64) -> Result<(), IntegrationError> {
...
#[doc(hidden)]
pub fn validate_count(value: u64) -> Result<(), IntegrationError> {
...
#[doc(hidden)]
pub fn validate_digest(value: &Sha256Digest) -> Result<(), IntegrationError> {
...
#[doc(hidden)]
pub fn domain_digest(domain: &[u8], bytes: &[u8]) -> Sha256Digest {
...
#[doc(hidden)]
pub const fn invalid() -> IntegrationError {
...
#[doc(hidden)]
pub const fn corrupt() -> IntegrationError {
    IntegrationError::new(
