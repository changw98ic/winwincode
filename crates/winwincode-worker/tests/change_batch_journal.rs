...
fn assert_journal_version(database: &Path, expected: i64) {
...
fn assert_schema_v1_migrates() {
    let root = TestDirectory::new("schema-v1");
    drop(ChangeBatchJournal::open(root.path()).expect("create journal schema"));
    let database = root.path().join("change-batch.sqlite3");
    let connection = Connection::open(&database).expect("open migration fixture");
    connection
        .execute_batch("DROP TABLE change_batch_workspace_barrier; PRAGMA user_version = 1;")
        .expect("downgrade fixture to v1");
    drop(connection);

    drop(ChangeBatchJournal::open(root.path()).expect("migrate v1 journal"));
    assert_journal_version(&database, 5);
    let connection = Connection::open(&database).expect("inspect migrated journal");
...
fn assert_schema_v2_migrates() {
    let root = TestDirectory::new("schema-v2");
    drop(ChangeBatchJournal::open(root.path()).expect("create v5 journal"));
    let database = root.path().join("change-batch.sqlite3");
    ...
    drop(connection);
    drop(ChangeBatchJournal::open(root.path()).expect("migrate v2 journal"));
    assert_journal_version(&database, 5);
}

fn assert_schema_v3_migrates() {
    let root = TestDirectory::new("schema-v3");
    drop(ChangeBatchJournal::open(root.path()).expect("create v5 journal"));
    let database = root.path().join("change-batch.sqlite3");
    let connection = Connection::open(&database).expect("open v3 fixture");
    connection
        .execute_batch(
            "DROP TABLE change_batch_observation_chunk;
             DROP TABLE change_batch_observation;
             DROP TABLE change_batch_diagnostic_evaluation;
             DROP TABLE change_batch_diagnostic_baseline;
             ALTER TABLE change_batch_phase_command DROP COLUMN diagnostic_parse_failed;
             ALTER TABLE change_batch_phase_command DROP COLUMN diagnostic_batch_json;
             PRAGMA user_version = 3;",
        )
        .expect("downgrade fixture to v3");
    drop(connection);
    drop(ChangeBatchJournal::open(root.path()).expect("migrate v3 journal"));
    assert_journal_version(&database, 5);
}

fn assert_schema_v4_migrates() {
    let root = TestDirectory::new("schema-v4");
    drop(ChangeBatchJournal::open(root.path()).expect("create v5 journal"));
    let database = root.path().join("change-batch.sqlite3");
    ...
    drop(connection);
    drop(ChangeBatchJournal::open(root.path()).expect("migrate v4 journal"));
    assert_journal_version(&database, 5);
}

fn assert_v5_missing_table_is_corrupt() {
    let root = TestDirectory::new("schema-v5-missing");
    drop(ChangeBatchJournal::open(root.path()).expect("create v5 journal"));
    let database = root.path().join("change-batch.sqlite3");
    let connection = Connection::open(&database).expect("open corrupt fixture");
    connection
        .execute("DROP TABLE change_batch_workspace_barrier", [])
        .expect("remove required v5 table");
    drop(connection);
    assert_eq!(
        ChangeBatchJournal::open(root.path())
            .expect_err("normal v5 open must not recreate a missing table")
            .code(),
        ChangeBatchJournalErrorCode::Corrupt
    );
}

fn assert_v5_obsolete_restore_key_is_corrupt() {
    let root = TestDirectory::new("schema-v3-old-restore-key");
    drop(ChangeBatchJournal::open(root.path()).expect("create strict v5 journal"));
    let database = root.path().join("change-batch.sqlite3");
    let connection = Connection::open(&database).expect("open old restore fixture");
    connection
        .execute_batch(
    ...
    assert_eq!(
        ChangeBatchJournal::open(root.path())
            .expect_err("normal v5 open rejects the obsolete restore key")
            .code(),
        ChangeBatchJournalErrorCode::Corrupt
    );
}
...
#[test]
fn schema_v1_through_v4_migrate_once_and_v5_open_does_not_repair_missing_tables() {
    assert_schema_v1_migrates();
    assert_schema_v2_migrates();
    assert_schema_v3_migrates();
    assert_schema_v4_migrates();
    assert_v5_missing_table_is_corrupt();
    assert_v5_obsolete_restore_key_is_corrupt();
}
