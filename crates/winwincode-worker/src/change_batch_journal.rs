...
use winwincode_domain::{ChangeBatchId, Instant, ModelExchangeId, Sha256Digest, WorkspaceRevision};
...
use winwincode_execution_port::{
    ...
    generated::{
        ...
        DiagnosticBaselineComparison, DiagnosticParserVersion, ExecutionOutcomeUsage,
        ExecutionPortMessage, ModelOpenMessage, NormalizedDiagnostic, NormalizerReceipt,
        ObservationReceipt, ObservationRequest, ValidationProfileSelection, ValidationReceipt,
    },
    observation_contract::{validate_observation_receipt, validate_observation_request},
...
impl ChangeBatchJournal {
    ...
    /// Atomically retains one exact one-shot Observer intent and appends
    /// `observation_requested` before any Provider side effect is allowed.
    ///
    /// # Errors
    ///
    /// Rejects unsafe input, changed replay bytes, foreign authority, a stale
    /// validated checkpoint, or non-contiguous progress.
    #[allow(clippy::too_many_lines)]
    pub fn retain_observation_request(
        ...
        if progress.state != ChangeBatchProgressState::ObservationRequested
            || progress.identity != intent.identity
            || now.0.is_empty()
        {
        ...
        let batch_id = &intent.identity.batch_id;
        let authority = load_authority(&self.connection, &batch_id.0)?
            .ok_or_else(|| conflict("ChangeBatch Observer has no retained intent"))?;
        if authority != intent.identity {
            return Err(conflict("ChangeBatch Observer authority is foreign"));
        }
        let transaction = self
            .connection
            .transaction()
            .map_err(|_| unavailable("ChangeBatch Observer transaction cannot begin"))?;
        let barrier = load_workspace_barrier_row(&transaction, workspace_id)?
            .map(|row| workspace_barrier_from_row(workspace_id, row))
            .transpose()?
            .ok_or_else(|| conflict("ChangeBatch Observer workspace barrier is missing"))?;
        let existing = transaction
            .query_row(
            ...
            .optional()
            .map_err(|_| unavailable("ChangeBatch Observer request cannot be read"))?;
        let existing_progress = transaction
            .query_row(
                "SELECT event_json FROM change_batch_progress
                 WHERE batch_id = ?1 AND sequence = ?2",
            ...
            .optional()
            .map_err(|_| unavailable("ChangeBatch Observer progress cannot be read"))?;
        if let Some((existing_request, existing_digest)) = existing {
            if existing_request == request_bytes
                && existing_digest == request_digest.0
                && existing_progress.as_deref() == Some(progress_bytes.as_slice())
                ...
                && barrier.active_batch_id.as_ref() == Some(batch_id)
            {
                transaction
                    .commit()
                    .map_err(|_| unavailable("ChangeBatch Observer replay cannot commit"))?;
                return Ok(JournalRetention::Replay);
            }
            return Err(conflict("ChangeBatch Observer request changed on replay"));
        }
        if barrier.state != ActiveBatchState::ValidationPending
            ...
            || existing_progress.is_some()
        {
            return Err(conflict("ChangeBatch Observer checkpoint is stale"));
        }
        ...
        let changed = transaction
            .execute(
                ...
                params![
                    workspace_id,
                    batch_id.0,
                    ActiveBatchState::ValidationPending.as_str(),
                    ActiveBatchState::ObservationPending.as_str(),
                    now.0,
                ],
            )
            .map_err(|_| unavailable("ChangeBatch Observer barrier cannot advance"))?;
        if changed != 1 {
            return Err(conflict(
                "ChangeBatch Observer barrier compare-and-set failed",
            ));
        }
        transaction
            .execute(
                "INSERT INTO change_batch_observation (
                   observation_id, batch_id, request_json, request_digest,
            ...
            .map_err(|_| unavailable("ChangeBatch Observer request cannot be persisted"))?;
        transaction
            .execute(
                "INSERT INTO change_batch_progress (batch_id, sequence, event_json, acknowledged)
                 VALUES (?1, ?2, ?3, 0)",
            ...
            .map_err(|_| unavailable("ChangeBatch Observer progress cannot be persisted"))?;
        transaction
            .commit()
            .map_err(|_| unavailable("ChangeBatch Observer request cannot commit"))?;
        Ok(JournalRetention::Inserted)
    }
    ...
    /// # Errors
    ///
    /// Rejects changed digests, non-canonical bytes, or invalid intent state.
    pub fn observation_request(
        &self,
        batch_id: &ChangeBatchId,
    ) -> Result<Option<ObservationRequest>, ChangeBatchJournalError> {
        ...
        if digest_bytes(&bytes).0 != digest {
            return Err(corrupt("ChangeBatch Observer request digest changed"));
        }
        let request: ObservationRequest = serde_json::from_slice(&bytes)
            .map_err(|_| corrupt("ChangeBatch Observer request is corrupt"))?;
        if canonical_bytes(&request)? != bytes
            || request.intent.identity.batch_id != *batch_id
            || validate_observation_request(&request).is_err()
        {
            return Err(corrupt("ChangeBatch Observer request is invalid"));
        }
        Ok(Some(request))
    }

    /// Retains the exact Provider-neutral model open after the Observer intent
    /// and before the message is sent to the Control Plane.
    ///
    /// # Errors
    ///
    /// Rejects foreign authority, changed replay bytes, invalid transport
    /// framing, or an observation without a durable intent.
    pub fn retain_observation_model_open(
        ...
        let request = self
            .observation_request(batch_id)?
            .ok_or_else(|| conflict("ChangeBatch Observer intent is missing"))?;
        ...
        let existing = self
            ...
            .optional()
            .map_err(|_| unavailable("ChangeBatch Observer model open cannot be read"))?
            .ok_or_else(|| conflict("ChangeBatch Observer intent is missing"))?;
        if let (Some(exchange), Some(existing_bytes)) = existing {
            ...
            return Err(conflict(
                "ChangeBatch Observer model open changed on replay",
            ));
        }
        let changed = self
            .connection
            ...
            .map_err(|_| unavailable("ChangeBatch Observer model open cannot be persisted"))?;
        if changed != 1 {
            return Err(conflict(
                "ChangeBatch Observer model open compare-and-set failed",
            ));
        }
        Ok(JournalRetention::Inserted)
    }

    /// Loads and validates durable Provider exchange state by model identity.
    ///
    /// # Errors
    ///
    /// Rejects corrupt canonical bytes, mismatched authority, or invalid
    /// bounded response/usage state.
    pub fn observation_model_record(
        &self,
        model_exchange_id: &ModelExchangeId,
    ) -> Result<Option<ObservationModelRecord>, ChangeBatchJournalError> {
        let Some((batch, open_bytes, confirmed, response, usage, terminal, receipt_bytes)) = self
            .connection
            .query_row(
            ...
            .optional()
            .map_err(|_| unavailable("ChangeBatch Observer model state cannot be read"))?
        else {
            return Ok(None);
        };
        ...
        let request = self
            .observation_request(&batch_id)?
            .ok_or_else(|| corrupt("ChangeBatch Observer intent is missing"))?;
        let model_open = open_bytes
            .map(|bytes| {
                let open: ModelOpenMessage = serde_json::from_slice(&bytes)
                    .map_err(|_| corrupt("ChangeBatch Observer model open is corrupt"))?;
                if canonical_bytes(&open)? != bytes
                    || open.model_exchange_id != *model_exchange_id
                    ...
                    .is_err()
                {
                    return Err(corrupt("ChangeBatch Observer model open is invalid"));
                }
                Ok(open)
            })
            .transpose()?;
        ...
        let model_usage = usage
            .map(|bytes| {
                let usage: ExecutionOutcomeUsage = serde_json::from_slice(&bytes)
                    .map_err(|_| corrupt("ChangeBatch Observer usage is corrupt"))?;
                if canonical_bytes(&usage)? != bytes
                    || usage.tokens < 0
                    || usage.runtime_millis < 0
                    || usage.cost_microunits < 0
                {
                    return Err(corrupt("ChangeBatch Observer usage is invalid"));
                }
                Ok(usage)
            })
            .transpose()?;
        let receipt = receipt_bytes
            .map(|bytes| {
                let receipt: ObservationReceipt = serde_json::from_slice(&bytes)
                    .map_err(|_| corrupt("ChangeBatch Observer receipt is corrupt"))?;
                if canonical_bytes(&receipt)? != bytes
                    || validate_observation_receipt(&receipt, &request.intent).is_err()
                {
                    return Err(corrupt("ChangeBatch Observer receipt is invalid"));
                }
                Ok(receipt)
            })
            .transpose()?;
        if !(0..=9_007_199_254_740_991).contains(&confirmed)
            || response_bytes.len()
                > winwincode_execution_port::observation_contract::MAX_OBSERVATION_RESPONSE_BYTES
            || terminal
                .as_deref()
                .is_some_and(|value| !matches!(value, "completed" | "provider_error"))
        {
            return Err(corrupt("ChangeBatch Observer model state is invalid"));
        }
    ...
    /// Loads the sole Observer exchange whose receipt is not yet durable.
    ///
    /// A terminal Provider frame and the canonical receipt are separate
    /// transactions. Including terminal exchanges here lets restart replay the
    /// original open and consume the Provider runtime's exact retained frames
    /// without invoking or billing the model again.
    ///
    /// # Errors
    ///
    /// Rejects corrupt state or more than one unfinished exchange per Job.
    pub fn pending_observation_model_open(
            ...
            .prepare(
                "SELECT observation.model_exchange_id
                 ...
                 WHERE observation.model_exchange_id IS NOT NULL
                   AND (
                     observation.receipt_json IS NULL
                     OR EXISTS (
                       SELECT 1 FROM change_batch_workspace_barrier AS barrier
                       WHERE barrier.active_batch_id = observation.batch_id
                         AND barrier.state = 'observation_pending'
                     )
                   )
                 ORDER BY created_at, observation_id LIMIT 1025",
            )
            .map_err(|_| unavailable("ChangeBatch Observer pending state cannot be opened"))?;
        let exchanges = statement
            .query_map([], |row| row.get::<_, String>(0))
            .map_err(|_| unavailable("ChangeBatch Observer pending state cannot be read"))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|_| unavailable("ChangeBatch Observer pending exchange cannot be read"))?;
        if exchanges.len() > MAX_RECOVERY_RECORDS {
            return Err(capacity(
                "ChangeBatch Observer pending exchange backlog is too large",
            ));
        }
        let mut matching = Vec::new();
        for exchange in exchanges {
            let record = self
                .observation_model_record(&ModelExchangeId(exchange))?
                .ok_or_else(|| corrupt("ChangeBatch Observer pending state disappeared"))?;
        ...
        if matching.len() > 1 {
            return Err(corrupt(
                "ChangeBatch Job has multiple unfinished Observer exchanges",
            ));
        }
        Ok(matching.pop())
    }
    ...
    /// # Errors
    ///
    /// Rejects changed replay state or an unavailable durable write.
    pub fn cancel_observation_model(
        ...
        let terminal = self
            .connection
            .query_row(
                "SELECT terminal_status FROM change_batch_observation WHERE batch_id = ?1",
                params![batch_id.0],
                |row| row.get::<_, Option<String>>(0),
            )
            .optional()
            .map_err(|_| unavailable("ChangeBatch Observer cancellation cannot be read"))?
            .ok_or_else(|| conflict("ChangeBatch Observer cancellation intent is missing"))?;
        if terminal.as_deref() == Some("provider_error") {
            return Ok(JournalRetention::Replay);
        }
        if terminal.is_some() || now.0.is_empty() {
            return Err(conflict("ChangeBatch Observer is already terminal"));
        }
        let changed = self
            .connection
            .execute(
            ...
            .map_err(|_| unavailable("ChangeBatch Observer cancellation cannot be persisted"))?;
        if changed != 1 {
            return Err(conflict(
                "ChangeBatch Observer cancellation compare-and-set failed",
            ));
        }
        Ok(JournalRetention::Inserted)
    }

    /// Loads a previously retained cancellation for exact acknowledgement replay.
    ///
    /// # Errors
    ///
    /// Rejects corrupt state or more than one cancelled exchange per Job.
    pub fn cancelled_observation_model_open(
        ...
        let mut statement = self
            ...
            .prepare(
                "SELECT model_exchange_id FROM change_batch_observation
                 ...
                 ORDER BY updated_at DESC, observation_id LIMIT 1025",
            )
            .map_err(|_| unavailable("ChangeBatch Observer cancellation cannot be opened"))?;
        let exchanges = statement
            .query_map([], |row| row.get::<_, String>(0))
            .map_err(|_| unavailable("ChangeBatch Observer cancellation cannot be read"))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|_| unavailable("ChangeBatch Observer cancellation is unavailable"))?;
        if exchanges.len() > MAX_RECOVERY_RECORDS {
            return Err(capacity(
                "ChangeBatch Observer cancellation backlog is too large",
            ));
        }
        let mut matching = Vec::new();
        for exchange in exchanges {
            let record = self
                .observation_model_record(&ModelExchangeId(exchange))?
                .ok_or_else(|| corrupt("ChangeBatch Observer cancellation disappeared"))?;
            if record.request.intent.identity.job_id == *job_id {
                matching.push(record);
            }
        }
        if matching.len() > 1 {
            return Err(corrupt(
                "ChangeBatch Job has multiple cancelled Observer exchanges",
            ));
        }
        Ok(matching.pop())
    }
    ...
    /// # Errors
    ///
    /// Rejects changed duplicate frames, a terminal stream continuation,
    /// oversized output, invalid usage, or an unknown exchange.
    ...
    pub fn retain_observation_model_chunk(
        ...
        now: &Instant,
    ) -> Result<ObservationChunkRetention, ChangeBatchJournalError> {
        if !(1..=9_007_199_254_740_991).contains(&sequence)
            || !valid_digest(chunk_digest)
            || now.0.is_empty()
            || model_usage.is_some_and(|usage| {
                usage.tokens < 0 || usage.runtime_millis < 0 || usage.cost_microunits < 0
            })
            || terminal_status.is_some_and(|value| !matches!(value, "completed" | "provider_error"))
        {
        ...
        let transaction = self
            .connection
            .transaction()
            .map_err(|_| unavailable("ChangeBatch Observer chunk transaction cannot begin"))?;
        let Some((observation_id, confirmed, existing_response, existing_terminal)) = transaction
            ...
            return Err(conflict("ChangeBatch Observer model exchange is unknown"));
        };
        if let Some(existing_digest) = transaction
            ...
            .optional()
            .map_err(|_| unavailable("ChangeBatch Observer chunk replay cannot be read"))?
        {
            if existing_digest != chunk_digest.0 || sequence > confirmed {
                return Err(conflict("ChangeBatch Observer chunk changed on replay"));
            }
            transaction
                .commit()
                .map_err(|_| unavailable("ChangeBatch Observer chunk replay cannot commit"))?;
        ...
        if sequence <= confirmed {
            return Err(corrupt(
                "ChangeBatch Observer confirmed chunk fingerprint is missing",
            ));
        }
        if sequence != confirmed.saturating_add(1) {
            transaction
                .commit()
                .map_err(|_| unavailable("ChangeBatch Observer gap cannot commit"))?;
            return Ok(ObservationChunkRetention::Gap {
                confirmed_sequence: confirmed,
            });
        }
        if existing_terminal.is_some() {
            return Err(conflict("ChangeBatch Observer stream is already terminal"));
        }
        ...
        transaction
            ...
            .map_err(|_| unavailable("ChangeBatch Observer chunk cannot be persisted"))?;
        let changed = transaction
            .execute(
                "UPDATE change_batch_observation
            ...
            .map_err(|_| unavailable("ChangeBatch Observer model cursor cannot advance"))?;
        if changed != 1 {
            return Err(conflict(
                "ChangeBatch Observer model cursor compare-and-set failed",
            ));
        }
        transaction
            .commit()
            .map_err(|_| unavailable("ChangeBatch Observer chunk cannot commit"))?;
    ...
    /// # Errors
    ///
    /// Rejects an invalid binding, nonterminal exchange, or changed replay.
    pub fn retain_observation_receipt(
        ...
        let request = self
            .observation_request(&receipt.identity.batch_id)?
            .ok_or_else(|| conflict("ChangeBatch Observer intent is missing"))?;
        validate_observation_receipt(receipt, &request.intent)
            .map_err(|_| invalid("ChangeBatch Observer receipt is invalid"))?;
        let batch_id = &receipt.identity.batch_id;
        let authority = load_authority(&self.connection, &batch_id.0)?
            .ok_or_else(|| conflict("ChangeBatch Observer authority is missing"))?;
        let base_revision = load_base_revision(&self.connection, &batch_id.0)?
            .ok_or_else(|| conflict("ChangeBatch Observer base is missing"))?;
        let prior_bytes = self
            .connection
            .query_row(
                "SELECT receipt_json FROM change_batch_execution WHERE batch_id = ?1",
                params![batch_id.0],
                |row| row.get::<_, Option<Vec<u8>>>(0),
            )
            .map_err(|_| unavailable("ChangeBatch receipt cannot be read"))?
            .ok_or_else(|| conflict("ChangeBatch applied receipt is missing"))?;
        let mut upgraded: ChangeBatchReceipt = serde_json::from_slice(&prior_bytes)
            .map_err(|_| corrupt("ChangeBatch receipt is corrupt"))?;
        if upgraded.observation.as_ref() == Some(receipt) {
            return Ok(JournalRetention::Replay);
        }
        if upgraded.observation.is_some() || now.0.is_empty() {
            return Err(conflict("ChangeBatch Observer receipt changed on replay"));
        }
        upgraded.observation = Some(receipt.clone());
        validate_receipt_without_progress(&upgraded, &authority, &base_revision)?;
        let upgraded_bytes = canonical_bytes(&upgraded)?;
        let observer_bytes = canonical_bytes(receipt)?;
        let transaction = self
            .connection
            .transaction()
            .map_err(|_| unavailable("ChangeBatch Observer receipt transaction cannot begin"))?;
        let observer_changed = transaction
            ...
            .map_err(|_| unavailable("ChangeBatch Observer receipt cannot be persisted"))?;
        let execution_changed = transaction
            .execute(
                "UPDATE change_batch_execution SET receipt_json = ?3, updated_at = ?4
                 WHERE batch_id = ?1 AND receipt_json = ?2",
                params![batch_id.0, prior_bytes, upgraded_bytes, now.0],
            )
            .map_err(|_| unavailable("ChangeBatch receipt cannot be upgraded"))?;
        let outbox_changed = transaction
            .execute(
                ...
                params![batch_id.0, prior_bytes, upgraded_bytes],
            )
            .map_err(|_| unavailable("ChangeBatch receipt outbox cannot be upgraded"))?;
        if observer_changed != 1 || execution_changed != 1 || outbox_changed != 1 {
            return Err(conflict(
                "ChangeBatch Observer receipt compare-and-set failed",
            ));
        }
        transaction
            .commit()
            .map_err(|_| unavailable("ChangeBatch Observer receipt cannot commit"))?;
        Ok(JournalRetention::Inserted)
    }

    /// Accepts a checkpoint proved safe by deterministic validation without
    /// creating an Observer model request.
    ///
    /// Stale or foreign validation facts return `Stale` without changing the
    /// accepted tree or releasing the Writer barrier.
    ///
    /// # Errors
    ///
    /// Rejects unavailable or corrupt durable state.
    pub fn accept_validated_checkpoint(
        &mut self,
        workspace_id: &str,
        progress: &ChangeBatchProgressEvent,
        validated_revision: &WorkspaceRevision,
        validated_delta_digest: &Sha256Digest,
        now: &Instant,
    ) -> Result<ObservationGateResult, ChangeBatchJournalError> {
        self.accept_checkpoint(
            workspace_id,
    ...
    fn accept_checkpoint(
        ...
        observed_revision: &WorkspaceRevision,
        observed_delta_digest: &Sha256Digest,
        expected_state: ActiveBatchState,
        now: &Instant,
    ) -> Result<ObservationGateResult, ChangeBatchJournalError> {
        let progress_bytes = canonical_bytes(progress)?;
        validate_progress_contract(progress, &progress_bytes)?;
    ...
    #[allow(clippy::too_many_lines)]
    fn initialize_schema_v5(connection: &Connection) -> Result<(), ChangeBatchJournalError> {
        connection
            .execute_batch(
                "CREATE TABLE IF NOT EXISTS change_batch_execution (
                 ...
                 CREATE TABLE IF NOT EXISTS change_batch_observation_chunk (
                   ...
                   FOREIGN KEY (observation_id) REFERENCES change_batch_observation(observation_id)
                 );",
            )
        ...
        if let Some(record) = record.as_ref() {
                ...
                .and_then(|receipt| receipt.observation.as_ref())
            {
                let request = self
                    .observation_request(batch_id)?
                    .ok_or_else(|| corrupt("ChangeBatch Observer intent is missing"))?;
                validate_observation_receipt(observation, &request.intent)
                    .map_err(|_| corrupt("ChangeBatch Observer receipt semantics changed"))?;
            }
        }
        Ok(record)
    }
...
fn migrate_journal_schema(connection: &mut Connection) -> Result<(), ChangeBatchJournalError> {
    ...
    match version {
        JOURNAL_SCHEMA_VERSION => validate_schema_v5(connection),
        4 => {
            ...
            transaction
                .execute_batch(
                    "CREATE TABLE change_batch_observation (
                       observation_id TEXT PRIMARY KEY NOT NULL,
                       batch_id TEXT UNIQUE NOT NULL,
                       request_json BLOB NOT NULL,
                       request_digest TEXT NOT NULL,
                     ...
                     CREATE TABLE change_batch_observation_chunk (
                       observation_id TEXT NOT NULL,
                       sequence INTEGER NOT NULL,
                       chunk_digest TEXT NOT NULL,
                       PRIMARY KEY (observation_id, sequence),
                ...
                .map_err(|_| unavailable("ChangeBatch journal v4 migration cannot apply"))?;
            transaction
        ...
        0..=2 => {
            let transaction = connection
                .transaction()
                .map_err(|_| unavailable("ChangeBatch journal migration cannot begin"))?;
            ChangeBatchJournal::initialize_schema_v5(&transaction)?;
            transaction
                .pragma_update(None, "user_version", JOURNAL_SCHEMA_VERSION)
                .map_err(|_| unavailable("ChangeBatch journal version cannot be persisted"))?;
            transaction
...
fn validate_schema_v5(connection: &Connection) -> Result<(), ChangeBatchJournalError> {
    for table in [
        ...
        "change_batch_observation",
        "change_batch_observation_chunk",
    ] {
        let exists = connection
            ...
            .is_some();
        if !exists {
            return Err(corrupt("ChangeBatch journal v5 table is missing"));
        }
    }
    validate_observation_schema(connection)?;
...
fn validate_observation_schema(connection: &Connection) -> Result<(), ChangeBatchJournalError> {
    if table_column_shape(connection, "change_batch_observation")?
        != [
            ("observation_id".to_owned(), 1, 1),
            ("batch_id".to_owned(), 1, 0),
            ("request_json".to_owned(), 1, 0),
            ("request_digest".to_owned(), 1, 0),
            ("model_exchange_id".to_owned(), 0, 0),
            ("model_open_json".to_owned(), 0, 0),
            ("confirmed_sequence".to_owned(), 1, 0),
            ("response_json".to_owned(), 0, 0),
            ("model_usage_json".to_owned(), 0, 0),
            ("terminal_status".to_owned(), 0, 0),
            ("receipt_json".to_owned(), 0, 0),
            ("created_at".to_owned(), 1, 0),
            ("updated_at".to_owned(), 1, 0),
        ]
        || table_column_shape(connection, "change_batch_observation_chunk")?
            != [
                ("observation_id".to_owned(), 1, 1),
                ("sequence".to_owned(), 1, 2),
                ("chunk_digest".to_owned(), 1, 0),
            ]
    {
        return Err(corrupt("ChangeBatch Observer schema is invalid"));
    }
    Ok(())
}

fn table_column_shape(
    connection: &Connection,
    table: &str,
) -> Result<Vec<(String, i64, i64)>, ChangeBatchJournalError> {
    let mut statement = connection
        .prepare(&format!("PRAGMA table_info({table})"))
        .map_err(|_| corrupt("ChangeBatch journal table cannot be inspected"))?;
    statement
        .query_map([], |row| {
            Ok((
                row.get::<_, String>(1)?,
        ...
        .map_err(|_| corrupt("ChangeBatch journal table cannot be read"))?
        .collect::<Result<Vec<_>, _>>()
        .map_err(|_| corrupt("ChangeBatch journal column cannot be read"))
}
