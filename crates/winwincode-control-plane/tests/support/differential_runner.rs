            ...
            let (mut transition, rework_authorization) = if requested_stage == "plan-review" {
                ...
                let transition = prepare_solution_review_fixture(
                    ...
                    },
                    {
                        ...
                        FIXTURE_USER_ID.clone_into(&mut fixture.assigned_to);
                        fixture
                    },
                )
...
fn canonical_spec_input(spec: &Value, repository_id: &str) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "constraints": canonical_string_list(spec, "constraints")?,
        "goal": required_str(spec, "goal")?,
        "outOfScope": canonical_string_list(spec, "outOfScope")?,
        "publicationTarget": spec.get("publicationTarget").cloned().unwrap_or(Value::Null),
        "repositoryId": repository_id,
        "scope": canonical_string_list(spec, "scope")?,
        "sourceProductSessionId": spec.get("sourceProductSessionId").cloned().unwrap_or(Value::Null),
...
fn canonical_string_list(spec: &Value, field: &str) -> Result<Vec<String>, String> {
    spec.get(field)
        .and_then(Value::as_array)
        .ok_or_else(|| format!("spec {field} must be an array"))?
        .iter()
        .map(|entry| {
            entry
                .as_str()
...
fn execution_config_for_transition(
    ...
    let (candidate_ref, checkout_revision) = if intent.stage == DeliveryStage::Verifying {
        ...
        if verifying_candidate.is_some() {
            return Err("only a verification dispatch may carry the frozen candidate".to_owned());
        }
...
mod tests {
    ...
    #[allow(
        clippy::too_many_lines,
        reason = "the scenario seals every stage role write mode inside one frozen plan"
    )]
