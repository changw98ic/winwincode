...
use winwincode_api::generated::{
    CommandEnvelope, CommandName, DeliveryTaskBreakdownCreatePayload, Scope,
};
...
pub(crate) fn execute_create(
    ...
    if command.command != CommandName::DeliveryTaskBreakdownCreate {
        return Err(StorageError::invalid_input("wrong command"));
    }
    if !matches!(command.scope, Scope::RepositoryScope(_)) {
        return Err(StorageError::invalid_input(
            "delivery.task_breakdown.create requires repository scope",
        ));
    }
    ...
    if mutation.replayed {
        return Err(StorageError::invalid_input(
            "task-breakdown create replay is missing its scoped command receipt",
        ));
    }
    ...
    let changed = delivery_changed_event(
        ...
        "delivery-task-breakdown-create",
    )?;
