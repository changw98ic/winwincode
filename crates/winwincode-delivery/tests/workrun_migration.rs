...
#[test]
fn historical_evidence_and_completion_do_not_borrow_new_runtime_authority() {
    let source: serde_json::Value = serde_json::from_slice(&fixture()).unwrap();
    for change in 0..4 {
        let mut changed = source.clone();
        match change {
            0 => changed["evidence"][0]["sessionBindingId"] = "foreign-binding".into(),
            1 => changed["evidence"][0]["workRunId"] = "wrn_01J00000000000000000000001".into(),
            2 => changed["verdict"] = serde_json::Value::Null,
            _ => changed["evidence"][0]["stageRunId"] = "foreign-stage".into(),
        }
        assert!(
            convert_canonical_delivery(&serde_json::to_vec(&changed).unwrap()).is_err(),
            "change {change}"
        );
    }
    let (_, output) = convert_canonical_delivery(&fixture()).unwrap();
    let output: serde_json::Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(output["historicalSource"], source);
}

#[test]
fn frozen_bindings_require_exact_task_and_valid_source_metadata() {
    let source: serde_json::Value = serde_json::from_slice(&fixture()).unwrap();
    for (field, invalid) in [
        ("deliveryTaskId", serde_json::Value::Null),
        ("schemaVersion", serde_json::json!(99)),
        ("id", serde_json::json!("")),
        (
            "sourceProvenance",
            serde_json::json!({"kind": "execution-port", "reference": ""}),
        ),
        (
            "sourceProvenance",
            serde_json::json!({"kind": "invented", "reference": "message"}),
        ),
    ] {
        let mut changed = source.clone();
        changed["sessionBindings"][0][field] = invalid;
        assert!(
            matches!(
                convert_canonical_delivery(&serde_json::to_vec(&changed).unwrap()),
                Err(WorkRunMigrationError::InvalidInput(_))
            ),
            "malformed frozen binding field {field} must be rejected"
        );
    }
}

#[test]
fn frozen_binding_ids_are_unique_across_distinct_historical_stages() {
    let mut source: serde_json::Value = serde_json::from_slice(&fixture()).unwrap();
    let mut stage = source["stageRuns"][0].clone();
    stage["id"] = serde_json::json!("stage-verification-2");
    stage["attempt"] = serde_json::json!(2);
    source["stageRuns"].as_array_mut().unwrap().push(stage);
    let mut binding = source["sessionBindings"][0].clone();
    binding["stageRunId"] = serde_json::json!("stage-verification-2");
    binding["attempt"] = serde_json::json!(2);
    binding["executionJobId"] = serde_json::json!("execution-job-verifier-2");
    source["sessionBindings"]
        .as_array_mut()
        .unwrap()
        .push(binding);
    assert!(matches!(
        convert_canonical_delivery(&serde_json::to_vec(&source).unwrap()),
        Err(WorkRunMigrationError::InvalidInput(_))
    ));
    source["sessionBindings"][1]["id"] = serde_json::json!("binding-verifier-2");
    convert_canonical_delivery(&serde_json::to_vec(&source).unwrap())
        .expect("distinct historical bindings retain their original stages");
}
