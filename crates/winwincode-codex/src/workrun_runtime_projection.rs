...
pub struct WorkRunRuntimeContext<'identity> {
...
pub enum WorkRunRuntimeRetention {
    Ready {
...
#[derive(Debug)]
pub enum WorkRunRuntimeProjectionError<AuthorityError, StoreError> {
...
impl<AuthorityError: fmt::Debug, StoreError: fmt::Debug> fmt::Display
    for WorkRunRuntimeProjectionError<AuthorityError, StoreError>
{
...
pub struct WorkRunRuntimeProjector {
...
impl WorkRunRuntimeProjector {
    ...
    ) -> Result<WorkRunRuntimeRetention, WorkRunRuntimeProjectionError<A::Error, S::Error>>
    where
        S: ReplayStore,
        A: ReplayAuthority<Context = RuntimeReplayIdentity>,
    {
        ...
        A: ReplayAuthority<Context = RuntimeReplayIdentity>,
    {
        ...
        let message = if let Some(frame) = existing {
            ...
            let sequence = snapshot
                .highest_sequence
                .checked_add(1)
                .ok_or(WorkRunRuntimeProjectionError::InvalidSource)?;
            build_message(context, event_id, sequence, projection.product)?
        };
        let decision = self
            .responder
            .retain_runtime_event(store, authority, &message)
            .map_err(WorkRunRuntimeProjectionError::Replay)?;
        Ok(match decision {
            ReplayDecision::Accepted { .. } => WorkRunRuntimeRetention::Ready {
            ...
            } => WorkRunRuntimeRetention::Gap {
                highest_sequence,
                replay_from_sequence,
            },
            ReplayDecision::Conflict { highest_sequence } => {
                WorkRunRuntimeRetention::Conflict { highest_sequence }
            }
        })
    }
}
...
fn build_message<AuthorityError, StoreError>(
    ...
    sequence: u64,
    product: &PreparedStageProduct,
) -> Result<RuntimeEventMessage, WorkRunRuntimeProjectionError<AuthorityError, StoreError>> {
    let sequence =
        i64::try_from(sequence).map_err(|_| WorkRunRuntimeProjectionError::InvalidSource)?;
...
fn replay_identity(context: WorkRunRuntimeContext<'_>) -> RuntimeReplayIdentity {
...
) -> Result<ReplaySnapshot, WorkRunRuntimeProjectionError<AuthorityError, S::Error>>
where
    S: ReplayStore,
{
    store
        ...
        .map_err(WorkRunRuntimeProjectionError::Store)
}
...
fn evidence_catalog<AuthorityError, StoreError>(
    ...
    HashMap<String, EvidenceBinding>,
...
> {
    ...
    for frame in &snapshot.events {
        ...
        let payload = message
            ...
            .ok_or(WorkRunRuntimeProjectionError::CorruptOriginal)?;
...
fn validate_source_id<AuthorityError, StoreError>(
    value: &str,
) -> Result<(), WorkRunRuntimeProjectionError<AuthorityError, StoreError>> {
    if value.is_empty()
    ...
    {
        Err(WorkRunRuntimeProjectionError::InvalidSource)
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    impl Fixture {
        fn context(&self) -> WorkRunRuntimeContext<'_> {
    ...
    fn ready(retention: Option<WorkRunRuntimeRetention>) -> Box<RuntimeEventMessage> {
        let Some(WorkRunRuntimeRetention::Ready { message, .. }) = retention else {
    ...
    fn retain_verification_sequence(
        projector: &WorkRunRuntimeProjector,
    ...
    fn planner_final_is_strict_and_restarts_with_original_frame() {
        ...
        let restarted = WorkRunRuntimeProjector::new();
        ...
        let Some(WorkRunRuntimeRetention::Ready {
            message,
            duplicate: true,
        }) = second
        else {
    ...
    fn verification_changed_or_foreign_evidence_fails_closed() {
        ...
        assert!(matches!(
            ...
            Some(WorkRunRuntimeRetention::Conflict { .. })
        ));
