...
use sha2::Sha256;
use winwincode_connector_github::{
    GITHUB_CONNECTOR_PROTOCOL, GitHubAppId, GitHubClock, GitHubConnector, GitHubConnectorConfig,
    GitHubCredentialError, GitHubCredentialPort, GitHubEventMapperPort, GitHubInboundEvent,
    GitHubInstallationId, GitHubInstallationPermissions, GitHubInstallationToken, GitHubPermission,
    GitHubTlsRoots, GitHubWebhookHeaders, GitHubWebhookRequestFactory, GitHubWebhookSecret,
    GitHubWebhookVerifier,
};
use winwincode_domain::{CredentialReferenceId, GitHubRepositorySlug};
use winwincode_integration_core::{
    ConnectorCallError, ConnectorCallErrorKind, ConnectorPort, ConnectorProtocol,
    ConnectorRegistration, InboundStatus, IntegrationErrorKind, IntegrationFramework,
    IntegrationId, IntegrationLeaseId, IntegrationOperationKey, IntegrationScope,
    NormalizedInboundEvent, OutboundAttemptResult, OutboundOperationState, OutboundRequest,
    RetryPolicy,
};
...
#[test]
fn signed_webhooks_are_integration_scoped_resource_ordered_and_secret_safe() {
    ...
    let mut connector =
        GitHubConnector::try_new(config.clone(), credentials, mapper, FixedClock(100))
            .expect("connector");
    ...
    let database_path = root.join("integration.sqlite3");
    drop(framework);
...
fn tls_check_run_delivery_uses_stable_provider_idempotency_and_exact_replay() {
    ...
    let mut connector = GitHubConnector::try_new(
        config.clone(),
        credentials,
...
fn missing_permission_dead_letters_before_any_network_call() {
    ...
    let mut denied_connector = GitHubConnector::try_new(
        denied_config.clone(),
        denied_credentials,
        MapperFixture::default(),
        FixedClock(100),
