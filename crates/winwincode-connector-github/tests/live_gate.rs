...
use winwincode_connector_github::{
    ...
    GitHubCredentialError, GitHubCredentialErrorKind, GitHubCredentialPort, GitHubEventMapperPort,
    GitHubInboundEvent, GitHubInstallationId, GitHubInstallationPermissions,
    GitHubInstallationToken, GitHubPermission, GitHubTlsRoots, GitHubWebhookHeaders,
    GitHubWebhookRequestFactory, GitHubWebhookSecret, GitHubWebhookVerifier,
};
...
use winwincode_integration_core::{
    ...
    ConnectorRegistration, InboundStatus, IntegrationFramework, IntegrationLeaseId,
...
impl LiveConfigFile {
    ...
    fn validate(self, webhook_payload: Vec<u8>) -> Result<ValidatedConfig, GateError> {
        ...
        let integration_scope =
            IntegrationScope::try_new(format!("repository:{}", repository_scope.repository_id.0))
                .map_err(|_| GateError::new(GateErrorCode::InvalidConfiguration))?;
