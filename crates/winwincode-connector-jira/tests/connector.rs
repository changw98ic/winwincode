...
use sha2::Sha256;
use winwincode_connector_jira::{
    JIRA_CONNECTOR_PROTOCOL, JiraClock, JiraConnector, JiraConnectorConfig, JiraCredentialError,
    JiraCredentialErrorKind, JiraCredentialPort, JiraEventMapperPort, JiraInboundEvent,
...
impl JiraEventMapperPort for MapperFixture {
    fn map_event(
        &mut self,
        _authority: &winwincode_integration_core::ConnectorAuthority,
...
fn jira_webhook(
    ...
    payload: Vec<u8>,
    received_at: u64,
) -> winwincode_integration_core::InboundWebhookRequest {
...
fn signed_closed_webhook_set_is_scoped_ordered_replayed_and_secret_safe() {
    ...
    let mut connector =
        JiraConnector::try_new(config.clone(), credentials, mapper, FixedClock(100))
            .expect("connector");
    ...
    let audit = serde_json::to_string(
        &framework
            .storage()
            .audit_facts(&integration_scope(), config.integration_id(), 0, 50)
            .expect("audit"),
    )
    .expect("audit JSON");
...
fn assert_bad_signature_rejected(
    ...
    connector: &mut JiraConnector<CredentialFixture, MapperFixture, FixedClock>,
) {
...
) -> JiraConnector<CredentialFixture, MapperFixture, FixedClock> {
    JiraConnector::try_new(
        config,
        credentials,
...
fn issue_create_recovers_by_property_marker_without_a_second_remote_write() {
    ...
    let abandoned = first
        ...
        .expect("due claim");
    assert!(
        winwincode_integration_core::ConnectorPort::deliver_outbound(&mut connector, &abandoned)
    ...
    let recovered = restarted
        ...
        .expect("due recovery");
    let found =
        winwincode_integration_core::ConnectorPort::deliver_outbound(&mut connector, &recovered)
            .expect("marker lookup");
    assert!(!found.remote_write_performed());
    restarted
        .storage_mut()
        .record_success(&integration_scope(), &recovered, &found, 106)
    ...
    let database = fs::read(root.join("integration.sqlite3")).expect("database bytes");
...
fn assert_dead_letter_for_credentials(
    ...
    assert_eq!(
        framework
            .storage()
            .authority(&integration_scope(), config.integration_id())
            .expect("authority")
            .state(),
...
fn exercise_live_webhook_evidence(
    ...
    let mut connector = JiraConnector::try_new(
        config.clone(),
        credentials.clone(),
        mapper,
...
fn live_jira_cloud_issue_comment_webhook_retry_and_leak_gate() {
    ...
    assert!(
        winwincode_integration_core::ConnectorPort::deliver_outbound(&mut connector, &issue_claim)
    ...
    let issue_receipt = winwincode_integration_core::ConnectorPort::deliver_outbound(
        &mut connector,
        &recovered_issue,
    )
    ...
    assert!(
        winwincode_integration_core::ConnectorPort::deliver_outbound(
            &mut connector,
            &comment_claim
        )
    ...
    let comment_receipt = winwincode_integration_core::ConnectorPort::deliver_outbound(
        &mut connector,
        &recovered_comment,
    )
    ...
    let authority = framework
        .storage()
        .authority(&integration_scope(), connector_config.integration_id())
    ...
    let mut inbound_connector = JiraConnector::try_new(
        connector_config.clone(),
        credentials.clone(),
        MapperFixture::default(),
        FixedClock(now),
