...
use winwincode_connector_webhook::{
...
use winwincode_integration_core::{
    ...
    InboundStatus, InboundWebhookMetadata, IntegrationErrorKind, IntegrationFramework,
...
fn integration_scope(organization: char, repository: char) -> IntegrationScope {
    IntegrationScope::try_new(format!("scope:{organization}:{repository}"))
        .expect("integration scope")
}
...
fn registration(scope: IntegrationScope) -> ConnectorRegistration {
...
fn assert_single_canonical_dispatch(framework: &IntegrationFramework, scope: &IntegrationScope) {
...
fn canonical_hmac_mapping_replays_orders_isolates_scope_and_survives_restart() {
    ...
    let scope = integration_scope('E', 'F');
    ...
    framework
        .register_connector(&registration(scope.clone()))
        .expect("register");
    ...
    assert_single_canonical_dispatch(&framework, &scope);
...
fn mutual_tls_proof_is_windowed_and_exactly_authorized() {
    ...
    let scope = integration_scope('J', 'K');
    let credentials = CredentialFixture::default();
    let mut framework = IntegrationFramework::new(IntegrationStorage::open(&root).expect("store"));
    framework
        .register_connector(&registration(scope.clone()))
...
fn outbound_retry_reuses_exact_operation_and_pinned_public_destination() {
    ...
    let scope = integration_scope('M', 'N');
    let credentials = CredentialFixture::default();
...
fn endpoint_ssrf_and_response_size_fail_closed_into_dead_letter() {
    ...
    let scope = integration_scope('R', 'S');
    let credentials = CredentialFixture::default();
    let transport = TransportFixture::default();
