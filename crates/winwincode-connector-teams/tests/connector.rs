...
use winwincode_connector_teams::{
    MICROSOFT_TEAMS_CONNECTOR_PROTOCOL, TeamsChannelId, TeamsConnector, TeamsConnectorConfig,
    TeamsCredentialError, TeamsCredentialPort, TeamsGraphAccessToken, TeamsGraphCallError,
    TeamsGraphClientState, TeamsGraphHttpTransport, TeamsGraphMessageReceipt,
    TeamsGraphOutboundMessage, TeamsGraphTlsRoots, TeamsGraphTokenClaims,
...
use winwincode_integration_core::{
    ...
    IntegrationOperationKey, IntegrationScope, OutboundAttemptResult, OutboundOperationState,
...
fn integration_scope() -> IntegrationScope {
    IntegrationScope::try_new("scope:3:6").expect("integration scope")
}
        ...
        if let Some(message_id) = state.messages.get(&key) {
            return TeamsGraphMessageReceipt::try_new(message_id.clone(), false).map_err(|_| {
                TeamsGraphCallError::try_new(
                    winwincode_connector_teams::TeamsGraphCallErrorKind::Permanent,
                )
        ...
        match state.outcomes.pop_front().expect("sandbox outcome") {
            ...
            SandboxOutcome::LostResponseAfterWrite => {
                ...
                Err(TeamsGraphCallError::try_new(
                    winwincode_connector_teams::TeamsGraphCallErrorKind::Retryable,
                )
...
fn tls_graph_sandbox_delivers_scoped_adaptive_card_with_stable_marker() {
    ...
    let mut connector = TeamsConnector::new(config.clone(), CredentialFixture, transport);
...
fn graph_webhook_authenticates_identity_scope_and_deterministic_expiry() {
    ...
    let mut connector = TeamsConnector::new(config.clone(), CredentialFixture, sandbox);

    let payload = notification_payload(TENANT_ID, TEAM_ID, CHANNEL_ID, "approval.approve", 7, 90);
    let request = factory
        .build(integration_scope(), payload.clone(), 100)
    ...
    let dispatches = framework
        .storage()
        .inbound_dispatches(&integration_scope(), config.integration_id(), 0, 10)
        .expect("dispatches");
    ...
    assert_eq!(
        factory
            .build(integration_scope(), foreign, 101)
    ...
    let audit_json = serde_json::to_string(
        &framework
            .storage()
            .audit_facts(&integration_scope(), config.integration_id(), 0, 20)
            .expect("audit facts"),
    )
    .expect("audit JSON");
    for secret in [
    ...
    let database_path = directory.join("integration.sqlite3");
    drop(framework);
...
fn invalid_graph_proofs_and_foreign_channels_create_zero_dispatches() {
    ...
    let valid_request = factory
        .build(integration_scope(), valid_payload, 102)
...
fn assert_signature_rejected(
    ...
    request: &winwincode_integration_core::InboundWebhookRequest,
    token_tenant: &str,
) {
    let mut verifier = TeamsGraphWebhookVerifier::new(
        config.clone(),
...
fn outbound_is_idempotent_honors_429_and_dead_letters_after_restart() {
    ...
    let mut framework = restart_and_reconcile(framework, &directory, &config, &mut connector);
    assert_dead_letter_and_secret_audit(
        &mut framework,
        &directory,
        &config,
        &request,
...
fn assert_rate_limit_retry(
    framework: &mut IntegrationFramework,
    config: &TeamsConnectorConfig,
    connector: &mut TeamsConnector<CredentialFixture, SandboxTransport>,
) {
    let first = framework
        .deliver_next(
            &integration_scope(),
...
fn assert_dead_letter_and_secret_audit(
    ...
    assert_secret_safe_audit(framework, directory, config, sandbox_state);
}
...
fn assert_secret_safe_audit(
    ...
    let audit = framework
        .storage()
        .audit_facts(&integration_scope(), config.integration_id(), 0, 30)
        .expect("audit facts");
    assert!(
        audit
            .iter()
    ...
    let database = fs::read(directory.join("integration.sqlite3")).expect("database bytes");
