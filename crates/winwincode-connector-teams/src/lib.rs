...
impl TeamsGraphWebhookRequestFactory {
    ...
    /// # Errors
    ///
    /// Rejects batches, unsupported notifications, and foreign scope/team/channel scope.
    pub fn build(
        &self,
...
pub struct TeamsConnector<Credentials, Transport> {
    config: TeamsConnectorConfig,
    credentials: Credentials,
    transport: Transport,
}

impl<Credentials, Transport> TeamsConnector<Credentials, Transport> {
    #[must_use]
    pub const fn new(
        config: TeamsConnectorConfig,
        credentials: Credentials,
...
impl<Credentials: TeamsCredentialPort, Transport: TeamsGraphTransportPort> ConnectorPort
    for TeamsConnector<Credentials, Transport>
{
