...
use crate::client::ModelStreamTransport;
...
impl ThreadManager {
    ...
    /// Inject a host-owned model stream transport for every root and descendant thread.
    pub fn with_model_stream_transport(mut self, transport: Arc<dyn ModelStreamTransport>) -> Self {
        let Some(state) = Arc::get_mut(&mut self.state) else {
...
impl ThreadManagerState {
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> CodexResult<NewThread> {
        ...
        let (session, io) = Box::pin(Session::spawn(SessionSpawnArgs {
            ...
            model_stream_transport: self.model_stream_transport.clone(),
