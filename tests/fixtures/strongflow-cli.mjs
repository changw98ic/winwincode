// Test-only Delivery CLI adapter fixture. The production host entry is removed; adapter coverage remains here.
import { randomUUID } from 'node:crypto';
import { readFile } from 'node:fs/promises';
import { STRONGFLOW_DELIVERY_API_SCHEMA_VERSION, materializeStrongFlowDeliveryFailure, parseStrongFlowDeliveryRequest, parseStrongFlowDeliveryResponseForRequest, } from '../../packages/contracts/dist/index.js';
...
export function renderStrongFlowDeliveryCliHelp() {
    return [
...
function parseArguments(tokens) {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
        ...
        if (name.length === 0)
            throw new CliUsageError('选项名称不能为空。');
        let value;
        if (separator !== -1) {
...
function rejectUnknownFlags(parsed, allowed) {
    ...
    if (unknown !== undefined)
        throw new CliUsageError(`未知选项 --${unknown}。`);
}
function exactPositionals(parsed, count) {
...
function positiveInteger(value, name) {
    ...
    if (!Number.isSafeInteger(parsed))
        throw new CliUsageError(`--${name} 数值过大。`);
    return parsed;
}
async function readJson(path, label, io) {
    try {
        return JSON.parse(await io.readTextFile(path));
    }
    catch {
        throw new CliUsageError(`${label}不是可读取的 JSON。`);
    }
}
function request(operation, requestId, payload) {
...
async function buildRequest(action, parsed, io) {
    const operation = ACTION_OPERATIONS[action] ?? null;
    if (operation === null)
        throw new CliUsageError(`未知 Delivery 命令 ${action}。`);
        ...
        switch (operation) {
            ...
            case 'updateDeliverySpec': {
                ...
                return request(operation, requestId, {
                    ...
                    expectedRevision: positiveInteger(onlyFlag(parsed, 'expected-revision', { required: true }), 'expected-revision'),
                    spec: await readJson(onlyFlag(parsed, 'spec', { required: true }), '规格文件', io),
                });
            }
            case 'startStage': {
            ...
            case 'submitVerdict': {
                ...
                return request(operation, requestId, {
                    deliveryId: parsed.positionals[0],
                    expectedRevision: positiveInteger(onlyFlag(parsed, 'expected-revision', { required: true }), 'expected-revision'),
                    candidate: await readJson(onlyFlag(parsed, 'candidate', { required: true }), '候选版本文件', io),
                    runtimeEvents: await readJson(onlyFlag(parsed, 'runtime-events', { required: true }), '运行事件文件', io),
...
function writeResponse(io, response) {
...
export async function runStrongFlowCli(argv, invoker, overrides = {}) {
    ...
    let response;
    try {
        response = parseStrongFlowDeliveryResponseForRequest(requestValue, await invoker.invoke(requestValue, io.signal === undefined ? {} : { signal: io.signal }));
    }
    catch {
