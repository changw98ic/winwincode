...
function recoveryError(code, message, cause) {
    throw new DeliveryRecoveryError(code, message, cause === undefined ? undefined : { cause });
}
function activeStageRuns(delivery) {
    return delivery.stageRuns.filter(run => run.status === 'running' || run.status === 'waiting');
}
function bindingsByRun(delivery) {
...
function validateBindingShape(delivery) {
    const grouped = bindingsByRun(delivery);
    const dshOwners = new Map();
    ...
    for (const binding of delivery.sessionBindings) {
        if (binding.dshSessionId !== null
            ...
            if (owner !== undefined) {
                recoveryError('SESSION_BINDING_CONFLICT', `DSH session ${binding.dshSessionId} is bound by both ${owner} and ${binding.id}`);
            }
        ...
        if (binding.codexSessionId !== null) {
            ...
            if (owner !== undefined) {
                recoveryError('SESSION_BINDING_CONFLICT', `Codex session ${binding.codexSessionId} is bound by both ${owner} and ${binding.id}`);
            }
    ...
    for (const run of delivery.stageRuns) {
        ...
        if ((!active && bindings.length !== 1) || (active && bindings.length > 1)) {
            recoveryError('SESSION_BINDING_CONFLICT', `StageRun ${run.id} has ${String(bindings.length)} owning SessionBindings`);
        }
        ...
        if (run.actorType === 'codex'
            ...
            recoveryError('SESSION_BINDING_CONFLICT', `Codex StageRun ${run.id} requires one complete DSH and Codex SessionBinding`);
        }
        if (run.actorType === 'human'
            && (binding.dshSessionId === null || binding.codexSessionId !== null)) {
            recoveryError('SESSION_BINDING_CONFLICT', `human StageRun ${run.id} requires one DSH-only SessionBinding`);
        }
    }
    return grouped;
}
function validateActiveStage(delivery, active) {
    if (active.length > 1) {
    ...
    if (!stageMatches) {
        recoveryError('DELIVERY_STATE_CONFLICT', `active StageRun ${run.id} does not match Delivery status ${delivery.status}`);
    }
}
function validateCandidateVerdictIdentity(delivery, candidate) {
    const verdict = delivery.verdict;
    if (verdict === null || verdict.candidateRef === candidate.candidateRef)
        return;
    ...
    if (!supersedesVerdict) {
        recoveryError('CANDIDATE_CONFLICT', `Delivery ${delivery.id} candidate does not match its current DeliveryVerdict`);
    }
}
function runnableTask(delivery, acceptedStatuses) {
    if (delivery.tasks.length === 0)
        return null;
    const tasks = new Map(delivery.tasks.map(task => [task.id, task]));
    const task = delivery.tasks.find(entry => (acceptedStatuses.includes(entry.status)
        && entry.blockedByTaskIds.every(dependencyId => tasks.get(dependencyId)?.status === 'completed')));
    if (task === undefined) {
        recoveryError('DELIVERY_STATE_CONFLICT', `Delivery ${delivery.id} has no runnable task for ${delivery.status}`);
    }
    return task.id;
}
function startAction(delivery) {
    switch (delivery.status) {
        ...
        case 'needs-attention':
            return recoveryError('DELIVERY_STATE_CONFLICT', 'needs-attention Delivery has no unresolved blocking AttentionItem');
        case 'plan-review':
            return recoveryError('DELIVERY_STATE_CONFLICT', 'plan-review is not a canonical stored Delivery status');
    }
}
function activeSessionFields(run, binding) {
    if (binding.dshSessionId === null || binding.codexSessionId === null) {
        return recoveryError('SESSION_BINDING_CONFLICT', `active Codex StageRun ${run.id} has an incomplete SessionBinding`);
    }
    return Object.freeze({
        stageRunId: run.id,
...
function selectNextAction(delivery, active, groupedBindings, sessionsByBinding, liveSessions) {
    const run = active[0];
    ...
    const blockingAttention = delivery.attentionItems.filter(item => (item.blocking && item.status === 'open'));
    if (blockingAttention.length > 0) {
        if (delivery.status !== 'needs-attention') {
            return recoveryError('DELIVERY_STATE_CONFLICT', 'open blocking Attention exists outside needs-attention state');
        }
        return Object.freeze({
    ...
    if (run === undefined)
        return startAction(delivery);
    if (run.actorType === 'human') {
        return recoveryError('DELIVERY_STATE_CONFLICT', `active human StageRun ${run.id} has no unresolved blocking AttentionItem`);
    }
    ...
    if (session === undefined) {
        return recoveryError('SESSION_LEDGER_CONFLICT', `active StageRun ${run.id} has no rebuilt runtime session`);
    }
    ...
    if (session.dsh.status === 'awaiting_approval') {
        return recoveryError('SESSION_LEDGER_CONFLICT', `runtime session ${fields.dshSessionId} is awaiting an interaction that is absent`);
    }
...
function validateSettledRuntimeInteractions(delivery, sessionsByBinding) {
    const runs = new Map(delivery.stageRuns.map(run => [run.id, run]));
    for (const binding of delivery.sessionBindings) {
        ...
        if ((run.status !== 'running' && run.status !== 'waiting')
            ...
            recoveryError('DELIVERY_STATE_CONFLICT', `settled StageRun ${run.id} retains unresolved runtime interaction`);
        }
    }
}
/**
 ...
 * a Codex operation.
 */
export async function reconcileDeliveryAfterRestart(options) {
    if (!isRecord(options)
        ...
        return recoveryError('INVALID_RECOVERY_OPTIONS', 'delivery recovery requires a home, Delivery id, and Codex session query');
    }
    let deliveryId;
    try {
    ...
    try {
        ...
        return recoveryError('DELIVERY_STORE_FAILURE', `Delivery ${deliveryId} could not be rebuilt from its owner records`, error);
    }
    ...
    if (options.candidate !== undefined && options.candidate !== null) {
        try {
            ...
            return recoveryError('CANDIDATE_CONFLICT', `Delivery ${delivery.id} candidate is stale or belongs to another owner record`, error);
        }
    }
    const runs = new Map(delivery.stageRuns.map(run => [run.id, run]));
    const sessionViews = [];
    ...
    for (const binding of delivery.sessionBindings) {
        ...
        if (run.actorType !== 'codex')
            continue;
        if (binding.dshSessionId === null || binding.codexSessionId === null) {
            return recoveryError('SESSION_BINDING_CONFLICT', `Codex StageRun ${run.id} has an incomplete SessionBinding`);
        }
        try {
            ...
            if (manifest.roleId !== run.role
                ...
                return recoveryError('SESSION_LEDGER_CONFLICT', `runtime ledger ${binding.dshSessionId} conflicts with SessionBinding ${binding.id}`);
            }
            ...
            if (error instanceof DeliveryRecoveryError)
                throw error;
            return recoveryError('SESSION_LEDGER_CONFLICT', `runtime ledger for SessionBinding ${binding.id} could not be rebuilt`, error);
        }
    }
    let strongFlow;
    try {
        ...
        return recoveryError('SESSION_LEDGER_CONFLICT', `StrongFlow runtime view for Delivery ${delivery.id} could not be rebuilt`, error);
    }
    ...
    try {
        ...
        return recoveryError('CODEX_SESSION_QUERY_FAILED', 'Codex session state could not be queried through its public boundary', error);
    }
    if (!Array.isArray(queriedSessions)
        ...
        return recoveryError('CODEX_SESSION_QUERY_FAILED', 'Codex returned an invalid session list');
    }
    const liveSessions = new Set(queriedSessions);
    const boundCodexSessions = new Set(delivery.sessionBindings.flatMap(binding => (binding.codexSessionId === null ? [] : [binding.codexSessionId])));
    const liveBoundCodexSessionIds = [...liveSessions]
        .filter(sessionId => boundCodexSessions.has(sessionId))
        .sort();
    const nextAction = selectNextAction(delivery, active, groupedBindings, sessionsByBinding, liveSessions);
    return immutable({
