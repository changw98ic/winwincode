...
function ledgerDirectory(home, sessionId) {
...
function manifestFrom(base, lifecycle) {
    return Object.freeze({
        schemaVersion: RUNTIME_SESSION_LEDGER_SCHEMA_VERSION,
...
export class RuntimeSessionLedger {
    ...
    static async create(options) {
        ...
        try {
            ...
            throw new RuntimeSessionLedgerError('INVALID_LEDGER_OPTIONS', error instanceof Error ? error.message : 'invalid ledger options');
        }
        const directory = ledgerDirectory(validated.home, validated.dshSessionId);
        const manifestPath = join(directory, 'manifest.json');
        await mkdir(dirname(directory), { recursive: true, mode: 0o700 });
        if (await pathExists(manifestPath)) {
            throw new RuntimeSessionLedgerError('LEDGER_ALREADY_EXISTS', `runtime ledger already exists for DSH session ${validated.dshSessionId}`);
        }
    ...
    static async open(home, dshSessionId) {
        try {
            nonEmpty(home, 'home');
        ...
        const manifestPath = join(directory, 'manifest.json');
        if (!(await pathExists(manifestPath))) {
            throw new RuntimeSessionLedgerError('LEDGER_NOT_FOUND', `runtime ledger was not found for DSH session ${dshSessionId}`);
        }
        try {
            const manifest = parseManifest(JSON.parse(await readFile(manifestPath, 'utf8')));
            if (manifest.dshSessionId !== dshSessionId) {
                throw new RuntimeSessionLedgerError('LEDGER_SESSION_MISMATCH', `runtime ledger belongs to DSH session ${manifest.dshSessionId}, not ${dshSessionId}`);
            }
            const ledger = new RuntimeSessionLedger(directory, manifest);
            const snapshot = await ledger.read();
            const latestLifecycle = snapshot.records.findLast((record) => record.recordType === 'kernel.lifecycle');
            if (latestLifecycle === undefined)
                throw new Error('ledger has no kernel lifecycle record');
            ...
            if (error instanceof RuntimeSessionLedgerError)
                throw error;
            throw new RuntimeSessionLedgerError('LEDGER_CORRUPT', `runtime ledger for DSH session ${dshSessionId} is corrupt`, { cause: error });
        }
    }
    get manifest() {
        return structuredClone(this.#manifest);
    }
    async appendLifecycle(lifecycle) {
    ...
    async appendEvent(event) {
        await this.#serialize(async () => {
            if (event.cursor.sessionId !== this.#manifest.dshSessionId
                ...
                throw new RuntimeSessionLedgerError('LEDGER_SESSION_MISMATCH', `runtime event ${event.id} does not belong to DSH session ${this.#manifest.dshSessionId}`);
            }
            if (event.source.kernelSessionId !== this.#manifest.kernelSessionId) {
                throw new RuntimeSessionLedgerError('LEDGER_SESSION_MISMATCH', `runtime event ${event.id} does not belong to active kernel session ${this.#manifest.kernelSessionId}`);
            }
            ...
            if (BigInt(event.cursor.sequence) !== expected) {
                throw new RuntimeSessionLedgerError('LEDGER_SEQUENCE_MISMATCH', `runtime event ${event.id} has sequence ${event.cursor.sequence}; expected ${expected.toString()}`);
            }
    ...
    async #readUnlocked() {
        ...
        try {
            ...
            throw new RuntimeSessionLedgerError('LEDGER_CORRUPT', `runtime ledger records are missing for DSH session ${this.#manifest.dshSessionId}`, { cause: error });
        }
        const records = [];
        const events = [];
        try {
            ...
            throw new RuntimeSessionLedgerError('LEDGER_CORRUPT', `runtime ledger records are corrupt for DSH session ${this.#manifest.dshSessionId}`, { cause: error });
        }
        return Object.freeze({
