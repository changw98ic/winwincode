...
test('independent Client build produces one deterministic target-neutral application package', () => {
  ...
  assert.equal(
    assetManifest.assets.some(asset => /enterprise/u.test(asset.path)),
    false,
    'Community package must not ship Enterprise route chunks',
  )
})
...
test('Client shell has one facade and the five canonical product entries with Home first', () => {
  ...
  assert.doesNotMatch(surfaces, /id: 'enterprise'/u)
  assert.doesNotMatch(index, /enterprise-/u)
  assert.doesNotMatch(application, /enterprise-application/u)
