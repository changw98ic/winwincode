...
test('real browser preserves drafts and gives safe recovery diagnostics across connection failures', async t => {
  ...
  assert.equal(result.revokedStatus.status, 'permission-denied')
  assert.match(result.revokedDiagnostic, /code=PERMISSION_REVOKED/iu)
  assert.doesNotMatch(result.revokedDiagnostic, /SECRET_TOKEN|private\/repository/iu)
