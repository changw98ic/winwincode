...
test('the viewer degrades binary, encoding, error, truncation, and empty states clearly', () => {
  ...
  assert.equal(findByClass(failure.viewer.root, 'wwc-candidate-diff-table')
    .getAttribute('data-columns'), '4')
...
test('the default 300-row window never leaves an actionable render-more no-op', () => {
  ...
  assert.equal(renderMore.textContent, 'Render 200 more Diff rows')
  renderMore.click()
  assert.equal(rowNodes(viewer).length, 500)
  assert.match(findByClass(viewer.root, 'wwc-candidate-diff-status').textContent,
    /renders at most 500 rows/u)
  assert.equal(renderMore.hidden || renderMore.disabled, true,
    'reaching the 500-row safety ceiling must not leave an actionable no-op button')
})

test('the 500-row message appears only when the safety ceiling still omits rows', () => {
  const content = [
    '@@ -1,350 +1,350 @@',
    ...Array.from({ length: 350 }, (_, index) => `-const old${String(index + 1)} = 1`),
    ...Array.from({ length: 350 }, (_, index) => `+const new${String(index + 1)} = 1`),
    '',
  ].join('\n')
  const { viewer } = mountViewer({ content, viewMode: 'side-by-side' })
  const renderMore = findByClass(viewer.root, 'wwc-candidate-diff-render-more')
  assert.equal(rowNodes(viewer).length, 300)
  assert.equal(renderMore.textContent, 'Render 51 more Diff rows')
  renderMore.click()
  assert.equal(rowNodes(viewer).length, 351)
  assert.equal(renderMore.hidden, true)
  assert.doesNotMatch(findByClass(viewer.root, 'wwc-candidate-diff-status').textContent,
    /renders at most 500 rows/u,
    'reaching the numeric ceiling is not a truncation when every presentation row fits')
})
