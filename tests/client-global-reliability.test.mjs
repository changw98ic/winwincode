...
test('application routes and browser failures use one secret-safe boundary', () => {
  ...
  assert.match(application, /showRouteFailure\(error, 'HOME_ROUTE_FAILURE'\)/u)
  assert.match(application, /showRouteFailure\(error, 'SETTINGS_ROUTE_FAILURE'\)/u)
  assert.match(application, /addEventListener\('offline'/u)
  assert.match(application, /addEventListener\('unhandledrejection'/u)
  assert.doesNotMatch(application, /console\.|sessionStorage/u)
