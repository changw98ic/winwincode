...
const expectedDeliveryTests = Object.freeze([
  ...
  'recovery_rejects_stage_with_foreign_actor_or_role',
  'stage_advance_rejects_time_before_current_delivery_state',
...
const expectedControlPlaneTests = Object.freeze([
  ...
  'new_commit_rejects_a_foreign_durable_job_without_dispatch',
...
test('approved DeliveryTask graph is one durable product graph with fixed status changes', () => {
  ...
  assert.deepEqual(taskProtocol.breakdownApproval, {
    ...
    requiresCurrentApprovedSolutionReview: true,
    reviewSetDigestRequired: true,
    callerTaskFieldsAllowed: false,
    promotionSource: 'current-sealed-approved-solution-review.taskProposals',
    taskProposalsIncludedInDigest: true,
    plannerSuppliedOwnerAllowed: false,
    promotedOwner: null,
    promotedInitialStatus: 'pending',
    maximumTaskProposals: 200,
