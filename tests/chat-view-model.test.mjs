...
function defaults() {
  return new Map([
    ...
    ['settings.get', response('settings.get', {
      revision: 3,
...
test('initial HTTP snapshot publishes one stable Chat state before subscribing at its cursor', async () => {
  ...
  assert.equal(model.state.defaultModelRoute.modelId, 'model')
...
test('reset clears the old view, reloads all snapshots, and hands back the new HTTP cursor', async () => {
  ...
  client.enqueue('settings.get', client.responses.get('settings.get'))
