...
import type { RuntimeSessionProjection } from './generated/contracts.js'
import {
  boundedItems,
  strongFlowElement,
...
export function mountStrongFlowActivityTimeline(options: {
  ...
  let evidenceIndex = new Map<string, string>()

  function evidenceIdFor(activity: RuntimeActivity): string | null {
...
export function mountStrongFlowExecutionGraph(
  ...
  function updateSessionView(
    ...
    const usage = session.usage === null || session.usage.totals.length === 0
      ? '—'
      : session.usage.totals
          .map(total => `${total.name} ${String(total.value)}`)
          .join(' · ')
    const recovery = session.recovery
    const failures = recovery.state === 'none' && recovery.failureCount === 0
      ? 'none'
      : `${recovery.state} · ${String(recovery.failureCount)} failures · ${String(recovery.recoveryCount)} recoveries`
    const outcomeValues = [
      ...
      session.diffSummary === null
        ? '—'
        : `${String(session.diffSummary.changedFileCount)} files · +${String(session.diffSummary.additions)} / −${String(session.diffSummary.deletions)} (counts only)`,
      usage,
      failures,
    ]
