...
export interface StrongFlowHeaderView {
  ...
  update(state: StrongFlowViewModelState, transitionStatus?: string): void
  close(): void
}
...
export function mountStrongFlowHeader(options: StrongFlowHeaderOptions): StrongFlowHeaderView {
  ...
  return {
    ...
    update(state, transitionStatus) {
      ...
      // While a retained projection exists alongside an in-flight transition,
      // the page's transition line is the single polite announcement; the
      // header keeps its visual next-step but stops being live until the
      // transition settles.
      status.hidden = typeof transitionStatus === 'string' && transitionStatus.length > 0
