...
const FAILURE_COPY: Readonly<Record<ClientFailureCategory, Readonly<{
  ...
  authentication: Object.freeze({
    title: '会话已过期',
    message: '请重新登录以恢复访问。此浏览器视图中未保存的内容会继续保留。',
    recoveryLabel: '重新登录',
  }),
  permission: Object.freeze({
    title: '访问权限已变化',
    message: '当前身份已无此区域的访问权限，请返回允许访问的区域。',
    recoveryLabel: '返回对话',
  }),
  version: Object.freeze({
    title: '客户端与服务器版本不一致',
    message: '请更新客户端后再重试此服务器操作。',
    recoveryLabel: '返回对话',
  }),
  network: Object.freeze({
    title: '连接已中断',
    message: '客户端重新连接期间会保留当前视图。',
    recoveryLabel: '重新连接',
  }),
  configuration: Object.freeze({
    title: '客户端配置需要处理',
    message: '请检查本地运行时配置后重试。',
    recoveryLabel: '重试当前页面',
  }),
  server: Object.freeze({
    title: '服务器操作失败',
    message: '请重试当前页面；如果再次失败，请复制诊断摘要。',
    recoveryLabel: '重试当前页面',
  }),
  protocol: Object.freeze({
    title: '服务器响应需要完整刷新',
    message: '请从当前服务器快照重新加载此页面。',
    recoveryLabel: '刷新当前页面',
  }),
  cancelled: Object.freeze({
    title: '操作已取消',
    message: '操作完成前页面已发生变化。',
    recoveryLabel: '重试当前页面',
  }),
  client: Object.freeze({
    title: '此区域意外停止',
    message: '请重试当前页面或返回对话。诊断详情不包含请求内容和本地路径。',
    recoveryLabel: '重试当前页面',
  }),
})
...
export function observeControlPlaneClient(
  ...
  const client: ControlPlaneClient = {
    ...
    changePassword(change, requestOptions) {
      return observe(options.client.changePassword(change, requestOptions))
    },
